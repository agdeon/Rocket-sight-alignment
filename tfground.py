...
def test(container_id):
    cont = ctypes.cast(container_id, ctypes.py_object).value
    print('h')
    rs = RSCreator.create()
    rs.draw_distance_marks(RocketTypeArg.EXP)
    rs.display()
    cont.window = rs.window
...
if __name__ == '__main__':
    ...
    container = Container()
    pr = multiprocessing.Process(target=test, args=(id(container),))
    pr.start()
    time.sleep(1)
